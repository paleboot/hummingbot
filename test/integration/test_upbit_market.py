...
class UpbitMarketUnitTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.market: UpbitMarket = UpbitMarket(
            f"{conf.upbit_api_key}",
            f"{conf.upbit_secret_key}",
            trading_pairs=["IDR-XRP", "IDR-ETH"]
        )
        # 2nd instance of market to prevent events mixing up across tests
        cls.market_2: UpbitMarket = UpbitMarket(
            f"{conf.upbit_api_key}",
            f"{conf.upbit_secret_key}",
            trading_pairs=["IDR-XRP", "IDR-ETH"]
        )
    ...
    def test_get_fee(self):
        limit_fee: TradeFee = self.market.get_fee("ETH", "IDR", OrderType.LIMIT, TradeType.BUY, 1, 2240000)
        self.assertGreater(limit_fee.percent, 0)
        self.assertEqual(len(limit_fee.flat_fees), 0)
        market_fee: TradeFee = self.market.get_fee("ETH", "IDR", OrderType.MARKET, TradeType.BUY, 1)
        self.assertGreater(market_fee.percent, 0)
        self.assertEqual(len(market_fee.flat_fees), 0)
        sell_trade_fee: TradeFee = self.market.get_fee("ETH", "IDR", OrderType.LIMIT, TradeType.SELL, 1, 2240000)
        self.assertGreater(sell_trade_fee.percent, 0)
        self.assertEqual(len(sell_trade_fee.flat_fees), 0)

    def test_limit_buy(self):
        trading_pair = "IDR-XRP"
        ...
        order_id = self.market.buy(trading_pair, quantized_amount, OrderType.LIMIT, Decimal(quantize_bid_price))
        ...
        self.assertEqual("IDR", order_completed_event.quote_asset)
    ...
    def test_limit_sell(self):
        trading_pair = "IDR-ETH"
        amount: Decimal = Decimal("0.00252062")
        quantized_amount: Decimal = self.market.quantize_order_amount(trading_pair, amount)
        ...
        order_id = self.market.sell(trading_pair, amount, OrderType.LIMIT, Decimal(quantize_ask_price))
    ...
    def test_market_buy(self):
        trading_pair = "IDR-XRP"
        # For placing bid order in market price, Upbit requires the total price of quote currency a user is willing to spend
        price: Decimal = Decimal("6000")
        quantized_price: Decimal = self.market.quantize_order_price(trading_pair, price)

        order_id = self.market.buy(trading_pair, 0, OrderType.MARKET, price)
        ...
        self.assertAlmostEqual(quantized_price, buy_order_completed_event.quote_asset_amount, places=4)
        self.assertEqual("XRP", buy_order_completed_event.base_asset)
        self.assertEqual("IDR", buy_order_completed_event.quote_asset)
    ...
    def test_market_sell(self):
        trading_pair = "IDR-ETH"
        # For placing ask order, Upbit requires the amount of base currency a user is willing to sell
        amount: Decimal = Decimal("0.00266654")
        quantized_amount: Decimal = self.market.quantize_order_amount(trading_pair, amount)
        ...
        self.assertEqual("IDR", sell_order_completed_event.quote_asset)
    ...
    def test_cancel_order(self):
        ...
        client_order_id = self.market.sell(trading_pair, quantized_amount, OrderType.LIMIT, quantize_bid_price)
    ...
    def test_cancel_all(self):
        ...
        bid_price: Decimal = self.market_2.get_price(trading_pair, True) * Decimal("1")
        ...
        # Intentionally setting a high price for ask and low price for bid to prevent orders getting filled
        quantize_bid_price: Decimal = self.market_2.quantize_order_price(trading_pair, bid_price * Decimal("0.7"))
        quantize_ask_price: Decimal = self.market_2.quantize_order_price(trading_pair, ask_price * Decimal("1"))
    ...
    def test_orders_saving_and_restoration(self):
        ...
        try:
            ...
            for event_tag in self.events:
                self.market.remove_listener(event_tag, self.market_logger)
            self.market: UpbitMarket = UpbitMarket(
                conf.upbit_api_key,
                conf.upbit_secret_key,
                trading_pairs = ["IDR-ETH", "IDR-XRP"]
            )
            for event_tag in self.events:
                self.market.add_listener(event_tag, self.market_logger)
            recorder.stop()
            ...
            print(saved_market_states.saved_state)
    ...
    def test_order_fill_record(self):
        ...
        try:
            # Try to buy 6916 IDR from the exchange, and watch for completion event.
            price: Decimal = Decimal("6916")
            order_id = self.market.buy(trading_pair, 0, OrderType.MARKET, price)
            [buy_order_completed_event] = self.run_parallel(self.market_logger.wait_for(BuyOrderCompletedEvent))
            ...
            # Try to sell the amount bought previously to the exchange, and watch for completion event.

...
class UpbitOrderBookTracker(OrderBookTracker):
    _upbitobt_logger: Optional[HummingbotLogger] = None

    @classmethod
    def logger(cls) -> HummingbotLogger:
        if cls._upbitobt_logger is None:
            cls._upbitobt_logger = logging.getLogger(__name__)
        return cls._upbitobt_logger
    ...
    def data_source(self) -> OrderBookTrackerDataSource:
        if not self._data_source:
            if self._data_source_type is OrderBookTrackerDataSourceType.EXCHANGE_API:
                self._data_source = UpbitAPIOrderBookDataSource(trading_pairs=self._trading_pairs)
            else:
        ...
        while True:
            try:
                ...
                if message.type is OrderBookMessageType.DIFF:
                    # Upbit websocket messages contain the entire order book state so they should be treated as snapshots
